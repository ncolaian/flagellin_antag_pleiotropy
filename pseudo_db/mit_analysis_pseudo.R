...
###########
#kate's mit file
mit_file <- read.delim("/Users/nicholascolaianni/Documents/dangl_lab/Kate_Vienna_data/pseudo_flic_data/mit_res/log20_res/mit.txt")
#mit_file <- read.delim("/Users/nicholascolaianni/Documents/dangl_lab/nat_variants_proj//882_data/updated_work_10_23_18/new_clusters/mit/clade1_mit.txt")
#msa_score <- read.delim("/Users/ncolaian/Documents/dangl_rotation/882_data/updated_work_7_20_18/mit/clade3_mit/zorr_out_CN_trimal.txt", header = F)
cluster <- 1
##################################################################
## ONE FUNCTION TO perform RCW, Z-SCORE, AND PVALUE CALCULATION ##
##################################################################
#mit_file <- run_analysis(mit_file, msa_score)
mit_file <- run_analysis(mit_file)

#Read in the actual location of each position on the fliC protein
col_position <- read.csv("/Users/nicholascolaianni/Documents/dangl_lab/Kate_Vienna_data/pseudo_flic_data/mit_res/positions_for_bact_of_interest.txt", header = T)
...
#write.csv(top,"/Users/nicholascolaianni/Documents/dangl_lab/Kate_Vienna_data/pseudo_flic_data/mit_res////top_hits.csv", row.names = F)
...
#mit_file$plot_Pos1 <- factor(as.character(mit_file$plot_Pos1), levels = as.character(col_position$X1) )
#mit_file$plot_Pos2 <- factor(as.character(mit_file$plot_Pos2), levels = as.character(col_position$X1) )
#mit_file$plot_Entropy <- mit_file$Entropy-quantile(mit_file$Entropy,.995)
#mit_file$plot_Entropy <- mit_file$Entropy-mean(mit_file$Entropy)
#mit_file$plot_Entropy <- mit_file$Entropy-quantile(mit_file$Entropy,.95)
...
#I want to display the flg22 region for each. Need to identify the flg22 regions
if ( cluster == 3 ) {
  flg22_pos <- c(28,42,50)
}

if ( cluster == 2 ) {
  flg22_pos <- c(27,41,49)
}
...
flg22_pos <- c(30,43,51)
ggplot(mit_file, aes(x=Pos2, y=Pos1))+
...
bonf_trial$breaks[bonf_trial$counts > 5]

write.csv(bonf_top, "/Users/nicholascolaianni/Documents/dangl_lab/882_data/updated_work_10_23_18/new_clusters/mit/c3_zscore.0005_mitres.csv", row.names = F, quote = F)

#get rid of the placed A's to separate C-term and N-term
mit_file <- mit_file[!(mit_file$Pos1 %in% a_positions) & !(mit_file$Pos2 %in% a_positions),]
